...
##                                        not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
        self.netG_A = networks.define_G(opt.input_nc, opt.input_nc, opt.ngf, 'our', opt.norm,
                                        not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
        # self.netG_B = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, 'our', opt.norm,
##                                        not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
    ...
    def calculateEdges(self, tensor,batch_size=4, lower = 10, upper = 210):
      ...
      for i in range(batch_size):
        ...
        imgrgb = cv2.GaussianBlur(gray, (3, 3), 0)

        out[i,:,:] = cv2.Canny(imgrgb, lower, upper)
    ...
    def set_input(self, input):
        ...
        self.L_A = self.real_A_org[:, 3:3+1, :, :].detach().clone()
        self.C_A = self.real_A_org[:, 3+1:3+1+3, :, :].detach().clone()         #real A LC
        

        # self.L_B = self.real_A_org[:, 3+10+3:3+10+3+10, :, :].detach().clone()       #augmented B LC
        self.L_B = self.real_B_org[:, 3:3+1, :, :].detach().clone()
    ...
    def forward(self):
        ...
        self.fake_B_edges = self.calculateEdges(self.fake_B)
        ...
        # self.i1_a, self.i2_a, self.i3_a, self.i4_a, self.i5_a, self.i6_a, self.i7_a, self.i8_a, self.i9_a, self.i10_a = self.netG_B(self.real_B_full)  # G_B(B)
    ...
    def backward_G(self):
        ...
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_features_edges + self.loss_idt_A + self.loss_idt_B
    ...
    def optimize_parameters(self):
        ...
        # forward
        # print("calling forward from optimize parameters")
